...
- 两只小爱音箱仍然作为语音控制入口；
- 实际音乐输出不再依赖某一只小爱音箱，而是统一输出到当前连接的蓝牙 A2DP 音频设备；
- 同一组内的设备共享播放状态、播放进度、播放列表和下一首任务；
- Web 设置页可以查看蓝牙 sidecar 状态，并执行 30 秒倒计时扫描、选择设备、连接、断开；
...
V5 仍保留 legacy command fallback，但推荐配置下不再依赖 `/app/bin/*.sh` 作为默认播放路径。
...
- `registry.cn-hangzhou.aliyuncs.com/aliyun_nas/xiaomusic-bluetooth:v5` 是发布到阿里云镜像仓库的主 XiaoMusic 蓝牙修复版镜像；
- `registry.cn-hangzhou.aliyuncs.com/aliyun_nas/xiaomusic-bluetooth:bluetooth-sidecar-systemd-v5` 是发布到阿里云镜像仓库的蓝牙 sidecar 镜像，由本仓库配套 sidecar 构建产物打包而来，不是上游官方镜像；
...
- 刷新 sidecar 状态；
- 默认 30 秒实时扫描蓝牙设备，并显示倒计时；
- 从扫描/已知设备列表中选择任意蓝牙设备；
- 连接所选设备；
- 断开当前或所选设备；
- 显示 sidecar 返回的 JSON 结果。

V5 中 sidecar 的连接接口支持 `address` 参数，成功连接后会更新 `current_address`，后续播放优先走该设备对应的 A2DP sink。能被扫描到的设备不一定能作为音频输出；需要具备 `Audio Sink` / A2DP profile 才能播放。
...
```text
/volume2/docker/Docker/xiaomusic
```
...
services:
  bt-audio-sidecar:
    ...
    environment:
      BT_TARGET_MAC: "44:F7:70:81:9C:C4" #自动连接的目标蓝牙地址，可在web配置里扫描得到
